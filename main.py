...
def send(__device_id, __install_id, cdid, openudid):
    ...
    for x in range(10):
        try:
            ...
            sig = Gorgon(
                params=params, cookies=None, data=None, unix=int(time.time())
            ).get_value()
            ...
            response = r.post(
                ...
                verify=False,
                proxies={"http": proxy_format + proxy, "https": proxy_format + proxy}
                ...
                if response.json()["status_code"] == 0:
                    ...
                    print(
                        Colorate.Horizontal(
                            ...
                            f"Hoàng Long Calisthenic | success: {success} fails: {fails} reqs: {reqs} rps: {rps} rpm: {rpm}",
                        )
                    )
                    success += 1
                    _lock.release()
...
def verify_key():
    correct_key = "HOANGLONGDZ"  # Thay đổi thành khóa chính xác của bạn
    while True:
        user_key = Write.Input("\n\n\tVui Lòng Nhập Key Để Tiếp Tục >> ", Colors.white_to_green, interval=0.0001)
        if user_key == correct_key:
            print(Col.green + "\n\n\tKey chính xác. Đang khởi động tool..." + Col.reset)
            break
        else:
            print(Col.red + "\n\n\tKey không chính xác. Vui lòng thử lại." + Col.reset)
...
if __name__ == "__main__":
    ...
    set_title("Hoàng Long Calisthenic")
    txt = """\n\n
    Hoàng Long Calisthenic \n"""
    print(
        Colorate.Vertical(
    ...
    try:
        ...
        os.system("cls" if os.name == "nt" else "clear")
        input(Col.red + "\n\n\t[x] - Liên kết không hợp lệ, hãy thử chỉ nhập id video" + Col.reset)
        sys.exit(0)
    ...
    while True:
        ...
        if eval(
            base64.b64decode(
                "dGhyZWFkaW5nLmFjdGl2ZV9jb3VudCgpIDwgMTAwICMgZG9uJ3QgY2hhbmdlIGNvdW50IG9yIHUgd2lsbCBraWxsIGRldmljZXMgYW5kIHJ1aW4gZnVuIGZvciBvdGhlcnM="
            )
        ):
            did, iid, cdid, openudid = device.split(":")
            eval(
                base64.b64decode(
                    "dGhyZWFkaW5nLlRocmVhZCh0YXJnZXQ9c2VuZCxhcmdzPVtkaWQsaWlkLGNkaWQsb3BlbnVkaWRdKS5zdGFydCgp"
                )
            )
